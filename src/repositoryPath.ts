...
): TFileSystemPath => join(options.path, id)

export const metaFolderName = (
  id: TEntryId,
  options: IRepositoryOptions
): TFileSystemPath =>
  join(dirname(this.fsPath(id, options)), options.metaFolderName)

export const metaFileName = (
  id: TEntryId,
  options: IRepositoryOptions
): TFileSystemPath => join(metaFolderName(id, options), `${basename(id)}.json`)
...
export const parentId = (id: TEntryId): string => dirname(id)
